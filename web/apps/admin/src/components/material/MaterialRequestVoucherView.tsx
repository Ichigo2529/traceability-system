...
/** SAP Fiori-style compact field for the document header */
function FieldBox({
    label,
    value,
    highlight = false,
    ...
    return (
        ...
        >
            ...
            <FlexBox
                ...
                style={{ width: "100%", paddingBottom: "1.25rem" }}
            >
            ...
            <div style={{ border: BORDER, borderRadius: "4px", overflow: "hidden", marginBottom: "1.25rem" }}>
                {/* Row 1: NO / DMI NO / DATE / (empty — aligns with COST CENTER) */}
                ...
                {/* Row 2: REQUESTOR / DEPT / SECTION / COST CENTER */}
            ...
            {/* ── Table Toolbar (SAP Fiori SmartTable pattern) ── */}
            <FlexBox
                justifyContent={FlexBoxJustifyContent.SpaceBetween}
            ...
            >
                {detail.items.length ? (
                    detail.items.flatMap((item) => {
                        ...
                        rows.push(
                            ...
                            >
                                ...
                                <TableCell>
                                    <ObjectStatus state="None">Requested</ObjectStatus>
                                </TableCell>
                                ...
                                <TableCell><Label style={{ color: "var(--sapContent_LabelColor)" }}>—</Label></TableCell>
                                <TableCell>
                                    <Label style={{ fontWeight: "bold", display: "block", textAlign: "right" }}>{item.requested_qty ?? "—"}</Label>
                                </TableCell>
                                <TableCell><Label style={{ fontSize: "0.82rem" }}>{item.uom || "PCS"}</Label></TableCell>
                                <TableCell>
                                    <Text style={{ fontStyle: "italic", fontSize: "0.75rem", color: "var(--sapContent_LabelColor)" }}>
                                        Requested Qty
                                    </Text>
                                </TableCell>
                                <TableCell />
                        ...
                        allocations.forEach((alloc, idx) => {
                            rows.push(
                                ...
                                >
                                    <TableCell>
                                        <Label style={{ color: "var(--sapInformativeElementColor)", fontSize: "0.82rem", fontWeight: "600" }}>
                                            {`${item.item_no}.${idx + 1}`}
                                        </Label>
                                    </TableCell>
                                    <TableCell>
                                        <ObjectStatus state="Information">DO Alloc</ObjectStatus>
                                    </TableCell>
                                    <TableCell><Label style={{ fontSize: "0.82rem" }}>{detail.model_code || "-"}</Label></TableCell>
                                    <TableCell><Label style={{ fontSize: "0.82rem" }}>{item.part_number || "-"}</Label></TableCell>
                                    <TableCell>
                                    ...
                                    <TableCell>
                                        <Label style={{ fontWeight: "bold", fontSize: "0.88rem", color: "var(--sapInformativeElementColor)" }}>
                                            {alloc.do_number || "-"}
                                        </Label>
                                    </TableCell>
                                    ...
                                    <TableCell>
                                        <Label style={{ fontWeight: "bold", display: "block", textAlign: "right" }}>{alloc.issued_qty ?? "—"}</Label>
                                    </TableCell>
                                    <TableCell><Label style={{ fontSize: "0.82rem" }}>{item.uom || "PCS"}</Label></TableCell>
                        ...
                        // ── Manual Store Allocation Rows (editable) ──
                        if (showIssueOptions && workbench && item.id) {
                            ...
                            manualRows.forEach((row, idx) => {
                                rows.push(
                                    ...
                                    >
                                        <TableCell>
                                            <Label style={{ color: "var(--sapCriticalElementColor)", fontSize: "0.82rem", fontWeight: "600" }}>
                                                {`${item.item_no}.${allocations.length + idx + 1}`}
                                            </Label>
                                        </TableCell>
                                        <TableCell>
                                            <ObjectStatus state="Critical">Store</ObjectStatus>
                                        </TableCell>
                                        <TableCell><Label style={{ fontSize: "0.82rem" }}>{detail.model_code || "-"}</Label></TableCell>
                                        <TableCell><Label style={{ fontSize: "0.82rem" }}>{item.part_number || "-"}</Label></TableCell>
                                        <TableCell>
                                        ...
                                        <TableCell>
                                            <Input
                                                ...
                                                onInput={(e) => workbench.setManualAllocations(prev =>
                                                    prev.map(x => x.id === row.id ? { ...x, issued_qty: Math.max(1, Number(e.target.value || 1)) } : x)
                                                )}
                                                style={{ width: "100%" }}
                                            />
                                        </TableCell>
                        ...
                        // ── Actual Total Row ──
                        const manualIssuedTotal = workbench && showIssueOptions && item.id
                            ? (workbench.allocationTotalsByItem[item.id] ?? 0)
                            : 0;
                        ...
                        rows.push(
                            ...
                            >
                                <TableCell>
                                    <Label style={{ fontWeight: "600", fontSize: "0.82rem" }}>{item.item_no}</Label>
                                </TableCell>
                                <TableCell>
                                    <ObjectStatus state="Positive">Total</ObjectStatus>
                                </TableCell>
                                <TableCell /><TableCell />
                                <TableCell>
                                    <Label style={{ fontWeight: "bold", fontSize: "0.85rem" }}>Actual Issued Total</Label>
                                </TableCell>
                                <TableCell /><TableCell /><TableCell /><TableCell />
                                <TableCell>
                                    <Label style={{ textAlign: "right", display: "block", fontWeight: "bold", fontSize: "0.95rem", color: absoluteTotal > 0 ? "var(--sapPositiveElementColor)" : undefined }}>
                                        {absoluteTotal || "—"}
                                    </Label>
                                </TableCell>
                        ...
                        rows.push(
                            <TableRow
                                key={`${key}-summary`}
                                style={{ background: "var(--sapGroup_TitleBackground)", borderBottom: "2px solid var(--sapGroup_ContentBorderColor)" }}
                            >
                                <TableCell /><TableCell /><TableCell /><TableCell />
                                <TableCell>
                                    <FlexBox alignItems={FlexBoxAlignItems.Center} style={{ gap: "0.5rem" }}>
                    ...
                    <TableRow>
                        <TableCell>
                            <Text style={{ textAlign: "center", width: "100%", padding: "1rem", color: "var(--sapContent_LabelColor)" }}>
                                No request items
                            </Text>
                        </TableCell>
            ...
            <Grid
                ...
                style={{ border: BORDER, borderRadius: "var(--sapElement_BorderCornerRadius)", overflow: "hidden", marginTop: "1.25rem" }}
            >
