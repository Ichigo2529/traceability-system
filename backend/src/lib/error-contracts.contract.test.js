...
describe("error contract", () => {
  it("does not use legacy error codes in event handlers", () => {
    ...
    const forbidden = ['"INVALID_TRANSITION"', '"INVALID_STATE"', '"INVALID_OUTER_STATE"', '"NOT_IMPLEMENTED"'];
  ...
  it("uses error_code in /events and /labels error responses", () => {
    ...
    const usesFailFromHttp = source.includes('from "../lib/http"') && source.includes("fail(");
    const usesInlineErrorCode = source.includes("error_code:");
    expect(usesFailFromHttp || usesInlineErrorCode).toBe(true);
